...
async function githubCommand(sock, chatId, message) {
    try {
        ...
        const res = await fetch('https://api.github.com/repos/Ebukannalue1132/CK SHADOW-MD');
        if (!res.ok) throw new Error(`GitHub API error: ${res.status} ${res.statusText}`);
        
        const json = await res.json();

        // Create professional formatted message
        let txt = `*🤖 CK SHADOW-MD REPOSITORY* 🤖\n\n`;
        txt += `📛 *Repository Name:* ${json.name || 'CK SHADOW-MD'}\n`;
        ...
        txt += `*© CK SHADOW 2026 | All Rights Reserved*`;
